...
class IssueActivity(ProjectBaseModel):
    ...
    # True when this activity entry was generated by the workflow rule
    # engine rather than a direct human/API action - see
    # docs/feature-specs/06-automation-workflow-sla.md ("Moteur de regles
    # d'automatisation") in plane-selfhost, and
    # plane/utils/workflow_rule_engine.py. Lets the activity feed render an
...
class IssueComment(ChangeTrackerMixin, ProjectBaseModel):
    ...
    # True when this comment was created by the workflow rule engine's
    # POST_COMMENT/MENTION_USER actions rather than a human - see
    # docs/feature-specs/06-automation-workflow-sla.md ("Moteur de regles
    # d'automatisation") in plane-selfhost. `actor`/`created_by` are still
