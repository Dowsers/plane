...
/**
 * Category 10, feature 4 ("Wiki workspace en GA") - workspace-level Wiki
 * page editor. Deliberately reuses the exact same `PageRoot` component
 * tree as the project-scoped page editor
 * (`(projects)/[projectId]/pages/(detail)/[pageId]/page.tsx`), only
 * swapping the store type (`EPageStoreType.WORKSPACE`), the collaboration
 * `documentType` ("workspace_page" instead of "project_page"), and
 * dropping `projectId` (undefined throughout - every downstream consumer
 * already treats it as optional, see this feature's own research notes).
 */
function WikiPageDetailsPage({ params }: Route.ComponentProps) {
  ...
  const pageRootHandlers: TPageRootHandlers = useMemo(
    () => ({
      create: createPage,
      fetchAllVersions: async (pid) => await workspacePageVersionService.fetchAllVersions(workspaceSlug, pid),
      ...
      fetchVersionDetails: async (pid, versionId) =>
        await workspacePageVersionService.fetchVersionById(workspaceSlug, pid, versionId),
      restoreVersion: async (pid, versionId) =>
        await workspacePageVersionService.restoreVersion(workspaceSlug, pid, versionId),
      getRedirectionLink: (pid) => {
        if (pid) {
          return `/${workspaceSlug}/wiki/${pid}`;
        } else {
