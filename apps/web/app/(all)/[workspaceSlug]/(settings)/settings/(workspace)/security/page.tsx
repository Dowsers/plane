 ...
 * 5 ("Role Owner dedie + Team/Project Owner delegue") merged - Workspace
 * Settings > Security. Two-tier gating, matching the backend exactly:
 * (1) nav-level - Admin-only (Member/Guest never even see the tab, see
 *     `WORKSPACE_SETTINGS.security` in @plane/constants);
 * (2) page-level - real-Owner-only content (`is_owner`, NOT a role) - a
 *     workspace Admin who isn't the Owner reaches this page but sees the
 *     `RestrictedToOwnerView` empty state instead of the audit log, mirroring
 *     the backend's own `IsWorkspaceOwner`-gated
