...
export function DeleteProjectModal(props: DeleteProjectModal) {
  ...
  const confirmDeletePhrase = t("project.delete_modal.confirm_phrase");
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setToast({
        ...
        message: t("project.delete_modal.success"),
      });
    } catch (_error) {
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("toast.error"),
        message: t("project.delete_modal.error"),
      });
    }
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6 p-6">
        ...
        <div className="text-secondary">
          <p className="text-13 break-words">
            {t("project.delete_modal.enter_name_prefix")}{" "}
            <span className="font-medium text-primary">{project?.name}</span>{" "}
            {t("project.delete_modal.enter_name_suffix")}
          </p>
          <Controller
            control={control}
            name="projectName"
        ...
        <div className="text-secondary">
          <p className="text-13">
            {t("project.delete_modal.confirm_phrase_prefix")}{" "}
            <span className="font-medium text-primary">{confirmDeletePhrase}</span>{" "}
            {t("project.delete_modal.confirm_phrase_suffix")}
          </p>
          <Controller
            control={control}
            name="confirmDelete"
            render={({ field: { value, onChange, ref } }) => (
              <Input
                ...
                placeholder={t("project.delete_modal.confirm_phrase_placeholder", { phrase: confirmDeletePhrase })}
                className="mt-2 w-full"
                autoComplete="off"
              />
            )}
