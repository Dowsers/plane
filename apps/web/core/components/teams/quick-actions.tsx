...
export const TeamQuickActions = observer(function TeamQuickActions(props: Props) {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteTeam(workspaceSlug.toString(), team.id);
      setToast({ type: TOAST_TYPE.SUCCESS, title: t("toast.success"), message: t("teams.toast.delete_success") });
  ...
  return (
    <>
      <CreateUpdateTeamModal isOpen={updateModal} handleClose={() => setUpdateModal(false)} team={team} />
      <AlertModalCore
        isOpen={deleteModal}
        handleClose={() => setDeleteModal(false)}
        handleSubmit={handleDelete}
        isSubmitting={isDeleting}
        title={t("teams.delete_confirm.title")}
        content={t("teams.delete_confirm.description")}
      />
      <CustomMenu
