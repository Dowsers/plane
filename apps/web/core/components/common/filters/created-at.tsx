...
export const FilterCreatedDate = observer(function FilterCreatedDate(props: Props) {
  ...
  const isCustomDateSelected = () => {
    ...
    return Boolean(isValidDateSelected.length > 0);
  };
    ...
    <>
      {isDateFilterModalOpen && (
        <DateFilterModal
          ...
          title={t("common_filters.created_date.title")}
        />
      )}
      <FilterHeader
        title={`${t("common_filters.created_date.title")}${appliedFiltersCount > 0 ? ` (${appliedFiltersCount})` : ""}`}
          ...
          {filteredOptions.length > 0 ? (
            <>
              ...
              <FilterOption
                ...
                title={t("common_filters.custom")}
                multiple
              />
            </>
          ) : (
            <p className="text-11 text-placeholder italic">{t("common_filters.no_matches_found")}</p>
          )}
        </div>
      )}
    </>
