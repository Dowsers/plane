...
export const getGroupByColumns = ({
  ...
  const groupByColumnMap: Record<GroupByColumnTypes, (columns: TGetColumns) => IGroupByColumn[] | undefined> = {
  ...
  return groupByColumnMap[groupBy]?.({ isWorkspaceLevel: isWsLevel, projectId });
};
...
const getLabelsColumns = ({ isWorkspaceLevel: isWsLevel }: TGetColumns): IGroupByColumn[] => {
  const { workspaceLabels, projectLabels } = store.label;
  // map labels to group by columns
  const labels = [
    ...(isWsLevel ? workspaceLabels || [] : projectLabels || []),
...
const getAssigneeColumns = ({ isWorkspaceLevel: isWsLevel, projectId }: TGetColumns): IGroupByColumn[] | undefined => {
  // store values
  const { getUserDetails } = store.memberRoot;
  // derived values
  const { memberIds, includeNone } = getScopeMemberIds({ isWorkspaceLevel: isWsLevel, projectId });
...
export const isDisplayFiltersApplied = (filters: Partial<IIssueFilters>): boolean => {
  ...
  const isDisplayFilterOptionsApplied = Object.keys(filters.displayFilters ?? {}).some((key) => {
  ...
  return isDisplayPropertiesApplied || isDisplayFilterOptionsApplied;
};
