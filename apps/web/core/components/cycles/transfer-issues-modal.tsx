...
export const TransferIssuesModal = observer(function TransferIssuesModal(props: Props) {
    ...
    await transferIssuesFromCycle(workspaceSlug.toString(), projectId.toString(), cycleId.toString(), payload)
      .then(async () => {
        setToast({
          ...
          title: t("cycle.transfer_issues_modal.toast.success.title"),
          message: t("cycle.transfer_issues_modal.toast.success.message"),
        });
      ...
      .catch(() => {
        setToast({
          ...
          title: t("cycle.transfer_issues_modal.toast.error.title"),
          message: t("cycle.transfer_issues_modal.toast.error.message"),
        });
      });
  };
  ...
  const getCycleDetails = async (newCycleId: string) => {
    ...
    await Promise.all(cyclesFetch).catch((error) => {
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("cycle.transfer_issues_modal.toast.fetch_error.title"),
        message: error.error || t("cycle.transfer_issues_modal.toast.fetch_error.message"),
      });
    });
  };
          ...
          {filteredOptions ? (
            filteredOptions.length > 0 ? (
              ...
              <div className="flex w-full items-center justify-center gap-4 p-5 text-13">
                ...
                <span className="text-center text-secondary">{t("cycle.transfer_issues_modal.empty_state")}</span>
              </div>
            )
          ) : (
