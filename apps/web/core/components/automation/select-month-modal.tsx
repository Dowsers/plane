      ...
      <form onSubmit={handleSubmit(onSubmit)}>
              ...
              {type === "auto-close" ? (
                <>
                  <Controller
                    ...
                    name="close_in"
                    rules={{
                      required: t("automation.select_month_modal.errors.range"),
                      min: 1,
                      max: 12,
                    }}
                    render={({ field: { value, onChange, ref } }) => (
                      <div className="relative flex w-full flex-col justify-center gap-1">
                        <Input
                          ...
                          placeholder={t("automation.select_month_modal.enter_months")}
                          className="w-full border-subtle"
                          min={1}
                          max={12}
                        />
                        <span className="absolute top-2.5 right-8 text-13 text-secondary">
                          {t("automation.select_month_modal.months")}
                        </span>
                      </div>
                    )}
                  />
                  ...
                  {errors.close_in && (
                    <span className="px-1 text-13 text-danger-primary">
                      {t("automation.select_month_modal.errors.range")}
                    </span>
                  )}
                </>
              ) : (
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <Button variant="primary" size="lg" type="submit" loading={isSubmitting}>
            {isSubmitting ? t("automation.select_month_modal.submitting") : t("submit")}
          </Button>
        </div>
      </form>
    </ModalCore>
